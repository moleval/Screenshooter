...
from .history import (HistoryManager, AddItemCommand, RemoveItemCommand,
                      MoveItemCommand, ChangePenCommand, CropCommand, RotateCommand, BlurCommand)
...
class EditorView(QGraphicsView):
    ...
    def __init__(self, scene):
        ...
        # Режимы редактирования изображения
        ...
        # Плавающий виджет для сообщений и разрешения
    ...
    def _update_after_history_change(self):
        ...
        if self.background_item and self.background_item.scene() is self.scene():
            self.setSceneRect(QRectF(self.background_item.pixmap().rect()))
        self._update_floating_widgets_visibility()

    # --------------------------------------------------------------
    # Установка фонового элемента
    # --------------------------------------------------------------
    def set_background_item(self, item):
        self.background_item = item
        self._disable_crop_mode_internal()
        self._disable_blur_mode_internal()
    ...
    def _disable_crop_mode_internal(self):
        self.crop_mode = False
    ...
    def _clear_crop_preview(self):
        if self.crop_rect_item and self.crop_rect_item.scene() is self.scene():
    ...
    def _show_crop_preview(self):
        if not self.crop_rect or self.crop_rect.isEmpty():
            return
        ...
        overlay_rects = [
            QRectF(scene_rect.left(), scene_rect.top(), scene_rect.width(), crop.top() - scene_rect.top()),
            QRectF(scene_rect.left(), crop.bottom(), scene_rect.width(), scene_rect.bottom() - crop.bottom()),
            QRectF(scene_rect.left(), crop.top(), crop.left() - scene_rect.left(), crop.height()),
            QRectF(crop.right(), crop.top(), scene_rect.right() - crop.right(), crop.height())
        ]

        for r in overlay_rects:
            if r.width() > 0 and r.height() > 0:
                overlay = QGraphicsRectItem(r)
                overlay.setPen(QPen(Qt.NoPen))
                overlay.setBrush(dim_color)
                overlay.setZValue(1000)
                overlay.setAcceptedMouseButtons(Qt.NoButton)   # не перехватывает мышь
                self.scene().addItem(overlay)
                self.crop_overlay_items.append(overlay)

        self.crop_rect_item = QGraphicsRectItem(crop)
        self.crop_rect_item.setPen(QPen(QColor(0, 120, 215), 2, Qt.DashLine))
        self.crop_rect_item.setBrush(QBrush(Qt.NoBrush))
        self.crop_rect_item.setZValue(1001)
        self.crop_rect_item.setAcceptedMouseButtons(Qt.NoButton)  # не перехватывает мышь
    ...
    def apply_crop(self):
        ...
        command = CropCommand(self.scene(), self.background_item, old_pixmap, new_pixmap, items_to_remove)
        ...
        self.setCursor(Qt.CrossCursor)
        self._update_floating_widgets_visibility()
        self.setSceneRect(QRectF(new_pixmap.rect()))
        self.viewport().update()
    ...
    def start_blur_mode(self):
        ...
        self.temp_crop_start = None
        self.setCursor(Qt.CrossCursor)
        self.blur_mode_changed.emit(True)
        self._update_floating_widgets_visibility()

    def _disable_blur_mode_internal(self):
        self.blur_mode = False
        self.setCursor(Qt.CrossCursor)
    ...
    def apply_blur(self, rect: QRectF):
        ...
        old_pixmap = self.background_item.pixmap()
        image_rect = old_pixmap.rect()
        blur_rect = rect.intersected(QRectF(image_rect))
        if blur_rect.isEmpty():
            return

        new_pixmap = blur_region(old_pixmap, blur_rect, radius=10.0)
        if new_pixmap.isNull():
            return

        command = BlurCommand(self.background_item, old_pixmap, new_pixmap)
        self.history.push(command)
    ...
    def rotate_image(self, angle: float):
        ...
        command = RotateCommand(self.scene(), self.background_item, old_pixmap, rotated_pixmap, items_to_remove)
        self.history.push(command)

        self.setSceneRect(QRectF(rotated_pixmap.rect()))
        self.viewport().update()
    ...
    def mousePressEvent(self, e):
        if self.crop_mode and e.button() == Qt.LeftButton:
            ...
            self._clear_crop_preview()
            self.crop_rect_item = QGraphicsRectItem(self.crop_rect)
            self.crop_rect_item.setPen(QPen(QColor(0, 120, 215), 2, Qt.DashLine))
            self.crop_rect_item.setBrush(QBrush(Qt.NoBrush))
            self.crop_rect_item.setZValue(1001)
            self.scene().addItem(self.crop_rect_item)
        ...
        if self.blur_mode and e.button() == Qt.LeftButton:
            sp = self.mapToScene(e.pos())
            self.temp_crop_start = sp
            self.crop_rect = QRectF(sp, sp)
            self.crop_rect_item = QGraphicsRectItem(self.crop_rect)
            self.crop_rect_item.setPen(QPen(QColor(0, 120, 215), 2, Qt.DashLine))
            self.crop_rect_item.setBrush(QBrush(Qt.NoBrush))
            self.crop_rect_item.setZValue(1000)
            self.scene().addItem(self.crop_rect_item)
            e.accept()
            return
    ...
    def mouseMoveEvent(self, e):
        if (self.crop_mode or self.blur_mode) and self.temp_crop_start is not None:
            sp = self.mapToScene(e.pos())
    ...
    def mouseReleaseEvent(self, e):
        if (self.crop_mode or self.blur_mode) and e.button() == Qt.LeftButton:
            if self.temp_crop_start is not None:
                ...
                if self.crop_mode:
                    ...
                    self.temp_crop_start = None   # фиксируем рамку
                elif self.blur_mode:
                    self.apply_blur(self.crop_rect)
                    self._clear_crop_preview()
                    self.crop_rect = None
                    self.temp_crop_start = None
                e.accept()
    ...
    def _update_floating_widgets_visibility(self):
        ...
        if self.crop_mode or self.blur_mode:
            return
