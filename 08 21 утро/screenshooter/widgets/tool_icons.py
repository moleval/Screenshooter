...
def create_arrow_mode_icon(mode):
    ...
    if mode == "straight":
        ...
        arrow_rotation_offset_deg = 35   # <- ручной поворот стрелки (в градусах)
        ...
        painter.drawLine(3, 15, 23, 15)   # было 13
        painter.setBrush(dimension_color)
        painter.drawPolygon(QPolygonF([QPointF(3,15), QPointF(8,10), QPointF(8,20)]))    # левая стрелка
        painter.drawPolygon(QPolygonF([QPointF(23,15), QPointF(18,10), QPointF(18,20)])) # правая стрелка
        painter.setPen(QPen(dimension_color, 2))
        painter.drawLine(3, 10, 3, 20)    # левая выносная
        painter.drawLine(23, 10, 23, 20)  # правая выносная (теперь симметрично)
        painter.setPen(QColor(50, 50, 50))
        painter.setFont(QFont("Arial", 10))
        painter.drawText(QRectF(5, 0, 16, 10), Qt.AlignCenter, "22")   # текст поднят выше
    painter.end()
    return QIcon(pixmap)
