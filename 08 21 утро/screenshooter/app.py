...
class ScreenshotApp(QMainWindow):
    ...
    def __init__(self):
        ...
        # Загрузка папки для быстрого сохранения из настроек
        ...
        # Левая группа кнопок: Undo, Redo, Снимок
        ...
        # Правая группа кнопок: В буфер, Сохранить как, Сохранить
        ...
        # Подключаем обновление кнопок Undo/Redo
        ...
        # Виджет с кнопками "Применить" и "Отмена" для режима обрезки
        ...
        # Добавляем crop_buttons_widget в центр верхней панели
        top_actions_layout.insertWidget(2, self.crop_buttons_widget)
        ...
        self.image_toolbar.setIconSize(QSize(26, 26))
        ...
        self.image_toolbar.setStyleSheet(
            "QToolButton { padding: 0px; margin: 0px; spacing: 0px; font-size: 8pt; min-width: 0px; max-width: 80px; } "
            "QToolButton:checked { background-color: #b0d4f1; border: 2px solid #005a9e; border-radius: 4px; }")
        ...
        # Скрываем стандартный статус-бар
        self.statusBar().hide()
        ...
        # Сигналы режимов обрезки/размытия
    ...
    def _on_crop_mode_changed(self, active):
        ...
        self._update_toolbar_actions_enabled()
    ...
    def _update_toolbar_actions_enabled(self):
        editing_mode = self.view.crop_mode or self.view.blur_mode
        self.rotate_cw_action.setEnabled(not editing_mode)
        self.rotate_ccw_action.setEnabled(not editing_mode)
        self.crop_action.setEnabled(not self.view.blur_mode)
        self.blur_action.setEnabled(not self.view.crop_mode)
