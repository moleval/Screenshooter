...
Описание: Основной виджет редактора (EditorView) на базе QGraphicsView.
          Обрабатывает создание аннотаций, выбор и перемещение объектов,
        ...
        if self._drag_item:
            ...
            if e.modifiers() & Qt.ShiftModifier:
                ...
                if abs(delta.x()) > abs(delta.y()):
                    new_pos.setY(self._drag_old_pos.y())
                else:
    ...
    def keyPressEvent(self, e):
        ...
        if e.key() in (Qt.Key_Left, Qt.Key_Right, Qt.Key_Up, Qt.Key_Down):
            ...
            if items:
                ...
                old_positions = [it.pos() for it in items]
                new_positions = []
                image_rect = None
                if self.image_editor.background_item is not None:
                    image_rect = QRectF(self.image_editor.background_item.pixmap().rect())

                for it, old_pos in zip(items, old_positions):
                ...
                self.history.push(MoveItemsCommand(items, old_positions, new_positions))
                e.accept()
                return
