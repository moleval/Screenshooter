...
def test_icon_manager_uses_semantic_colors(qapp):
    ...
    def nontransparent_pixels(pixmap):
    ...
    assert any(c.red() == 210 and c.green() == 81 and c.blue() == 69
               for c in nontransparent_pixels(annotation))
    assert any(c.red() == 0 and c.green() == 90 and c.blue() == 158
               for c in nontransparent_pixels(editing))
    assert any(c.red() == 51 and c.green() == 51 and c.blue() == 51
               for c in nontransparent_pixels(selection))


def test_icon_manager_disabled_icon_is_transparent(qapp):
    icon = IconManager.icon("rect")
    normal = icon.pixmap(20, 20, QIcon.Normal, QIcon.Off).toImage()
    disabled = icon.pixmap(20, 20, QIcon.Disabled, QIcon.Off).toImage()
