...
class FakeViewport:
    """Заглушка viewport с методом update()."""
    def update(self):
        pass


class FakeBlurController:
    """Заглушка для BlurController."""
    def __init__(self):
        self.blur_regions = []
...
class FakeHistory:
    """Заглушка HistoryManager с нужными методами."""
...
class FakeView:
    """Минимальная заглушка EditorView."""
    def __init__(self):
...
def test_copy_and_paste_rectangle(qapp, view, controller):
    ...
    assert len(items) == 2  # оригинал + копия
